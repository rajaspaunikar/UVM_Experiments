...
__global__ void addOne(int *data, long long n)
{
...
int main()
{
    ...
    std::cout << "Allocation successful. Initializing on CPU..." << std::endl;
    ...
    std::cout << "CPU initialization complete. Launching kernel..." << std::endl;

    int threadsPerBlock = 64;
    int blocks = (N + threadsPerBlock - 1) / threadsPerBlock;

    cudaEventRecord(start1,green_ctx_stream1);
    addOne<<<blocks, threadsPerBlock, 0, green_ctx_stream1>>>(data, N);
    cudaError_t err = cudaGetLastError(); 
    if (err != cudaSuccess) {
        printf("Launch error: %s\n", cudaGetErrorString(err));
        exit(EXIT_FAILURE);
    }
    std::cout << "Bully kernel launched." << std::endl;
    ...
    cout << "Bully kernel time: " << milliseconds << " ms\n";
