...
def process_and_plot(csv1, pid1, csv2, pid2, csv3, pid3, output_img="eviction_comparison_3way.png"):
    # 1. Load CSVs
    ...
    # 3. Figure Setup (Compact 8x5 size for display, 300 DPI for high-res PNG export)
    plt.style.use('seaborn-v0_8-whitegrid' if 'seaborn-v0_8-whitegrid' in plt.style.available else 'default')
    fig, ax = plt.subplots(figsize=(8, 5), dpi=100)

    # Styling Configurations for 3 Runs
    series_config = [
        (f1, pid1, '#1f77b4', '-', 'o', 8, f"CSV 1 (PID: {pid1})"),     # Solid Blue (Shift Text Up)
        (f2, pid2, '#d62728', '--', 's', -16, f"CSV 2 (PID: {pid2})"),   # Dashed Red (Shift Text Down)
        (f3, pid3, '#2ca02c', ':', '^', 20, f"CSV 3 (PID: {pid3})")      # Dotted Green (Shift Text Further Up)
    ]

    # 4. Plot Traces and Annotate 3 Key Points (Start, Middle, End)
    for df, pid, color, ls, marker, text_offset, label in series_config:
        if not df.empty:
            ax.plot(
                df['Iter'], 
                df['Memory Evicted of other Processes'], 
                marker=marker, 
                markersize=4,
                linestyle=ls, 
                linewidth=1.2, 
            ...
            # Annotate only Start, Middle, and End
            key_indices = get_key_indices(len(df))
            for idx in key_indices:
                row = df.iloc[idx]
                x, y = row['Iter'], row['Memory Evicted of other Processes']
                ax.annotate(
                    f"({int(x)}s, {int(y):,})",
                    xy=(x, y),
    ...
    # 5. Y-Axis Formatting (Real integers with commas instead of floats)
    ...
    ax.set_ylabel('Memory Evicted of other Processes', fontsize=10, fontweight='bold', labelpad=6)
    ax.set_title('UVM Memory Eviction Benchmark Comparison (Processor ID 1)', fontsize=11, fontweight='bold', pad=10)
    ...
    # 7. Save high-res PNG file
    plt.savefig(output_img, dpi=300, bbox_inches='tight')
    print(f"[+] High-resolution PNG saved to: {output_img}")
    plt.show()

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Compare UVM memory evictions across 3 CSV files into a static PNG.")
    parser.add_argument("--csv1", type=str, default="run1.csv", help="Path to first CSV")
    parser.add_argument("--pid1", type=int, default=115050, help="PID for first CSV")
    ...
    parser.add_argument("--out", type=str, default="eviction_comparison_3way.png", help="Output PNG file name")
