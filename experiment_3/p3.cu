/*
 * p3_touch.cu — P3: allocate 6GB managed memory, touch all pages 40 times,
 * pacing page-touches in small chunks with a short sleep between each chunk
 * so total wall time is ~25 seconds.
 *
 * Build: nvcc -O2 -arch=native p3_touch.cu -o p3_touch
 * Run:   ./p3_touch
 *        (or under a cgroup: ./run_in_cgroup.sh <cgroup_name> ./p3_touch)
 */
...
static const size_t SIZE_MB          = 2ULL * 1024ULL;  // 6 GB
