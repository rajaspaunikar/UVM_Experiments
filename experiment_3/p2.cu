/*
 * p2_touch.cu — P2: allocate 8GB managed memory, touch all pages 7 times,
 * pacing page-touches in small chunks with a short sleep between each chunk
 * so total wall time is ~25 seconds.
 *
 * Build: nvcc -O2 -arch=native p2_touch.cu -o p2_touch
 * Run:   ./p2_touch
 *        (or under a cgroup: ./run_in_cgroup.sh <cgroup_name> ./p2_touch)
 */
...
static const size_t SIZE_MB          = 4ULL * 1024ULL;  // 8 GB
